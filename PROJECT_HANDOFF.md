...
- **MULTILINGUAL LEVEL ESTIMATOR (equity headline; ArabLexify shape) - NOT YET, needs a download.** CHILDES has 20+ languages. Build the LEVEL estimator in a second language (e.g. Spanish). Level layer low-risk (MLU-age holds cross-linguistically); DELAY layer cross-lingually needs a clinical corpus in that language (the real reach). GATE: confirm a 2nd-language TD corpus parses + age-complexity signal holds before promising it. STRONGEST remaining ceiling-raiser.
