...
- **STATUS (2026-07-20): Stages 1-5 DONE; 9 extensions; core HARDENED (young-children gap fixed).** Core level estimator now MAE 9.99 mo, R2 0.81 on **335 distinct children** (Move A added Bates+Gleason -> 48 kids <48mo, was 3). Headline delay flag ROC-AUC 0.861 (age/length proven NOT the signal). Stage 5 demo. Extensions: growth-chart percentiles; CROSS-CORPUS external validation Conti4 (AUC 0.70, no refit); short-sample (AUC 0.82 @10 utts); fairness; calibration; item-level GRAMMATICAL PROFILE (subordinate clauses d=0.85, novelty); cross-task reliability (honest negative); MULTILINGUAL Spanish (gate passes, estimator weak - honest partial + cross-linguistic insight). Remaining ceiling-raisers = age-matched 2nd clinical corpus (Gillam) + a better/more diverse multilingual corpus + poster.
...
- **ISS-13 | 2026-07-20 | FINDING (honest partial) | Multilingual (Spanish) estimator is WEAK on available corpora.**
  - Saw: Spanish gate PASSES on wide-age BecaCESNo (corr 0.48) but the level estimator is weak (R2 0.23 vs English 0.81); Shiro FAILS (corr 0.23, age-plateau 6-11yr); pooling the two makes it worse (R2 0.05, task/dialect confound).
  - Why it matters: the multilingual "equity headline" is the intended ceiling-raiser; a weak result must not be oversold.
  - Diagnosis/positioning: the age<->complexity PREMISE transfers (gate passes) - concept validated cross-linguistically - but a strong estimator needs a task-consistent, wide-YOUNG-age, many-child Spanish corpus we don't have. Report as proof-of-concept + cross-linguistic insight (slope steepest early; length-based MLU weak on the plateau), not a finished screener. Same young=steep/old=flat pattern as our English data. `src/ext_multilingual.py`.
...
- **Conti4/** - Conti-Ramsden 4, British; 99 TD + 19 SLI ADOLESCENTS (13-16 yr); two tasks (frog narrative + spontaneous), group+task in FOLDER names. External cross-corpus test ONLY (Section 10 PART B); NOT in core train set. (Now under data/childes/Conti4/.)
- **Bates/** - English TD, ~28 children at 20 & 28 mo (folders Free20/Free28/Snack28/Story28 = conditions, same child recurs; child_id normalizes name via bates_child_id()). %mor present. IN the core TD set (Move A).
- **Gleason/** - English TD, ~24 children, ~25-62 mo (folders Dinner/Father/Mother = contexts, same child stem). %mor present. IN the core TD set (Move A).
- **Shiro/** - SPANISH TD, 113 children, 77-129 mo (6.4-11 yr), narratives. Multilingual test ONLY. GATE FAILS (corr age,MLU=0.23 - kids are on the age plateau). Honest negative.
- **BecaCESNo/** - SPANISH TD, 70 aged children, 35-146 mo (3-12 yr, wide). Multilingual test ONLY. GATE PASSES (corr 0.48) but estimator weak (MAE 21.6, R2 0.23) - see Section 10 multilingual.
...
- **MULTILINGUAL LEVEL ESTIMATOR (equity headline; ArabLexify shape). `src/ext_multilingual.py` HARNESS BUILT; awaiting data. STRONGEST remaining ceiling-raiser.**
  ...
  - **RECOMMENDED FIRST LANGUAGE = SPANISH.** Best corpora from the CHILDES Spanish list (verified from the collection table): **Shiro (N=113 children, ages 6;0-9;0, narratives - flagship: many children + task matches ENNI)** as primary, plus **BeCaCeSno (N=40, ages 3;6-11;6 - widest age range, guarantees the age<->complexity slope / gate)** as companion. Together ~153 children across 3.5-11.5 yr. (Avoid: Koine=Galician not Spanish; GRERLI age-range garbled; most others N<=3 or narrow age.) Unzip into data/ml/Spanish/Shiro/ and data/ml/Spanish/BeCaCeSno/. Harness gates each independently; report per-corpus (don't blindly pool - dialect/task differ). CAVEAT: level features need tokenized words only (fine), so %mor is NOT required for the multilingual level estimator.
  - **RESULT (2026-07-20, Shiro + BecaCESNo tested via `ext_multilingual.py`) - HONEST PARTIAL:**
    - BecaCESNo (3-12 yr, wide) GATE PASSES: corr(age, MLU-words)=0.48, level estimator MAE 21.6 mo (baseline 25.0), R2 0.23. Shiro (6-11 yr) GATE FAILS: corr 0.23 - kids are on the language PLATEAU (MLU by age-tercile 5.1->5.5->5.8, ~flat). Adding morphemes/pooling did NOT help (pooling R2 0.05 - task/dialect confound).
    - HONEST READ: the age<->complexity premise DOES transfer to Spanish (gate passes on wide-age data) = concept validated cross-linguistically. BUT the estimator is WEAK vs English (R2 0.23 vs 0.81), because the available Spanish corpora are either on the age-plateau (Shiro) or task-heterogeneous with sparse sampling (BecaCESNo). This mirrors our OWN English pattern (young/steep = strong, older/flat = weak) - a coherent, cross-linguistically consistent story, not a fluke.
    - HONEST POSITIONING: present as a "proof-of-concept that the growth-chart PREMISE is language-general + a cross-linguistic insight (the developmental slope is steepest early; length-based MLU weakens on the plateau and may need syntax/morphology-based measures in richer languages)." Do NOT oversell it as a finished Spanish screener. A STRONG Spanish estimator needs a corpus with a CONSISTENT task + WIDE YOUNG age (2-6yr) + many children - not available in what we have; that's the future-work ask.
  - Strategy for MORE languages: prefer typological DIVERSITY over count - Spanish (done, partial) + Cantonese/Mandarin (isolating) + German/Dutch (rich morphology), each gated. But NOTE the lesson from Spanish: corpus MATCH (task-consistent, young, many kids) matters more than language count. Chase the right corpus, not more flags.
