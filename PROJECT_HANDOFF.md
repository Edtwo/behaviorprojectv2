...
- **MULTILINGUAL LEVEL ESTIMATOR (equity headline; ArabLexify shape). `src/ext_multilingual.py` DONE 2026-07-20 - GERMAN WORKS; Spanish premise confirmed but data-limited.**
  ...
  - MORE languages: German succeeded because of DATA MATCH (many kids, wide young age, consistent). Chase that profile, not language count. Cantonese (HKU) or a large Dutch/German norm corpus are good next candidates; each gated the same way.
