...
- **STATUS (2026-07-20 pm): Stages 1-5 DONE; 9 extensions; core HARDENED + TWO external validations + a MULTILINGUAL WIN.** Core level estimator MAE 9.99 mo, R2 0.81 on **335 distinct children** (48 kids <48mo, was 3). Delay flag ROC-AUC 0.861 (age/length proven NOT the signal), and it EXTERNALLY VALIDATES with no refit on two independent clinical corpora: **Gillam (age-matched, US) AUC 0.80-0.82** and Conti4 (out-of-range, UK) 0.70. MULTILINGUAL: the growth-chart method REPRODUCES in **German (Szagun MAE 3.1 mo, R2 0.79)**; premise confirmed within-child in Spanish (r=0.77-0.99) but cross-child estimator data-limited (equity point). Plus growth-chart percentiles, short-sample (AUC 0.82 @10 utts), fairness, calibration, item-level grammar (subordinate clauses d=0.85). Superseded corpora archived in data/childes/not_in_use/. Remaining = poster (Section 9); optional more languages / clinical corpora.
...
- **MULTILINGUAL LEVEL ESTIMATOR (equity headline; ArabLexify shape). `src/ext_multilingual.py` DONE 2026-07-20 - GERMAN WORKS; Spanish premise confirmed but data-limited.**
  ...
  - **GERMAN - Szagun controls (22 children, 15-44 mo, densely + consistently sampled): STRONG. corr 0.85, within-child median 0.90, child-independent LEVEL estimator MAE 3.1 mo, R2 0.79** (~English quality). Fig results/ext_ml_deu_szagun.png. THIS is the multilingual win: the growth-chart method transfers to a second language family (German = Germanic/inflected) when the data has many children across a wide YOUNG age range.
