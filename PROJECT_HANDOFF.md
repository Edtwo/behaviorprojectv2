...
- **Move A (fix the young-children gap - HIGH priority):** add Eng-NA TD corpora with many DISTINCT children in the 18-48 mo range. Best picks that exist in CHILDES Eng-NA: **Bates (~27 children at 20 & 28 mo - directly fills the toddler gap)** + **Gleason (~24 children, ~2-5 yr, continuous spread, dinner/lab contexts)**. Combined with Brown -> ~50+ distinct young children (vs 3 now). Unzip into data/childes/Bates/ and data/childes/Gleason/. NOTE: Stage 2 requires mlum + syntax_index (need %mor/%gra tiers) - most Eng-NA corpora have MOR-run tiers, but if a corpus lacks them, either (a) relax Stage 2 to the language-universal features (mluw+mattr) for those files, or (b) run them through CLAN's MOR. Verify tiers on download; re-run stage2.
### Access
